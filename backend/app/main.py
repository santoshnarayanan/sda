...
import io
import zipfile
from app.models import DeploymentFile
...
@app.post("/api/v1/export_deployment_bundle")
async def export_deployment_bundle(files: List[DeploymentFile]):
    """
    Create a ZIP archive containing the deployment artifacts
    generated by the DevOps Agent.
    """
    ...
    with zipfile.ZipFile(in_memory_zip, mode="w", compression=zipfile.ZIP_DEFLATED) as zf:
        for file in files:
            ...
            zf.writestr(filename, file.content)
    ...
    return StreamingResponse(
        ...
        headers={
            "Content-Disposition": "attachment; filename=deployment_bundle.zip"
        }
    )
